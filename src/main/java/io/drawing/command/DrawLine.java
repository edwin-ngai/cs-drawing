 ...
 *
 */
public class DrawLine extends RenderingCommand {
	...
	public DrawLine(String[] arguments, CommandContext context) {
	...
	private void init(int x1, int y1, int x2, int y2, CommandContext context) {
		...
		Validate.isTrue((x1==x2&&y1!=y2) || (x1!=x2&&y1==y2), 
				"Given coordinates [(%d,%d), (%d, %d)] do not form a horizontal or vertical line.",
				x1, y1, x2, y2);
		//need to consider displacement between canvas and command coordinates
	...
	protected void doExecution() throws CommandExecutionException {
		...
		Line line = null;
		if (horizontal) {
			line = new HorizontalLine(new Point(x1, y1), new Point(x2, y2));
		}else {
			line = new VerticalLine(new Point(x1, y1), new Point(x2, y2));
		}
		if (!canvas.drawShape(line)) {
			throw new CommandExecutionException(
					String.format("Unknown error encountered during drawing line [(%d,%d), (%d,%d)]", x1, y1, x2, y2));
		}
